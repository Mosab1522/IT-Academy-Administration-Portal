    ...
    <x-setting heading="Vytvoriť typ kurzu" etitle="Vyberte inštruktora triede">
        ...
        <form action="/admin/classes/create" method="post">
            ...
            <div class="p-6 mb-4 bg-gray-100 rounded-md">
                ...
                <p><strong>Kurz:</strong> {{ $coursetype->name }}
                    {{ $classAttributes['type'] == 1 ? '- inštruktorský' : '- študentský' }}</p>
                <p><strong>Dni:</strong>
                    {{ $classAttributes['days'] == 1 ? 'Týžďeň' : ($classAttributes['days'] == 2 ? 'Víkend' : 'Nezáleží') }}
                </p>
                <p><strong>Čas:</strong>
                    {{ $classAttributes['time'] == 1 ? 'Ranný' : ($classAttributes['time'] == 2 ? 'Poobedný' : 'Nezáleží') }}
                </p>
            </div>

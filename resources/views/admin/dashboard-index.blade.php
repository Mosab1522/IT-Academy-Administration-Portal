                ...
                @php
                    if (auth()->user()->can('admin')) {
                        ...
                        $academy = \App\Models\Academy::whereHas('coursetypes.instructors', function ($query) use (
                            ...
                            ->get();
                        $coursetype = \App\Models\CourseType::whereHas('instructors', function ($query) use (
                            $authInstructorId,
                        ) {
                            $query->where('instructors.id', $authInstructorId);
                        })->get();
            ...
            <x-form.search-select name="coursetype_id" title="Kurz" class="combo-b4" disabled>
                ...
                @foreach ($coursetype as $type)
                    <option value="{{ $type->id }}" data-id="{{ $type->id }}"
                        ...
                        {{ request()->input('coursetype_id') == $type->id ? 'selected' : '' }}>
                        {{ ucwords($type->name) }}
                    </option>
                @endforeach
            </x-form.search-select>
                    ...
                    <x-single-table>
                        <x-slot:head>
                            ...
                            <th scope="col" class="py-3 px-6 w-48"> <a
                                    href="/admin/coursetypes/{{ $coursetype->id }}"

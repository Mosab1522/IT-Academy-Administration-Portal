            ...
            <div class="bg-white p-8 rounded-lg shadow-md mb-6">
                ...
                <form action="/admin/lessons/create" method="post">
                    ...
                    <x-form.field>
                        @php
                            if (auth()->user()->can('admin')) {
                                ...
                                $classes = \App\Models\CourseClass::with(['coursetype'])
                    ...
                    <div id="emailDiv" class="mt-6 {{ old('email') ? '' : 'hidden' }}">
                        ...
                        <div id="onsiteDiv" class="{{ old('onsite') ? '' : 'hidden' }} mt-6"><x-form.input
                                name="onsite" type="text" title="Miestnosť"
                                placeholder="Uveďte miestnosť vyučovania" /></div>
        ...
        @foreach ($lessons->groupBy('class.name') as $className => $lessonsInClass)
            ...
            <div class=" mb-6">
                ...
                <h3 class="-mb-4 leading-6  text-gray-700">Hodiny triedy -
                    <a href="/admin/lessons/{{ $classId }}"
                        class="hover:underline underline-offset-2 hover:text-gray-900">
                        {{ $className }}
                    </a>
                </h3>
                    ...
                    @foreach ($lessonsInClass as $lesson)
                        <tr
                            ...
                            <x-table.td-last url="lessons/{{ $lesson->id }}" edit=1
                                itemName="hodinu {{ $lesson->title }}? Spolu s tým sa vymažú záznamy o absolvovaní tejto hodiny." />
                        </tr>
                    @endforeach
                </x-single-table>
            </div>
